...
if(any(!file.exists(required))) {
  stop("Stage 3G base outputs missing. Run full Stage 3G first.", call.=FALSE)
}
...
fin <- read_csv(file.path(outdir,"01_financial_final_data.csv"),
                show_col_types=FALSE, progress=FALSE)
legal <- read_csv(file.path(outdir,"02_legal_final_data.csv"),
                  show_col_types=FALSE, progress=FALSE)
targets <- read_csv(file.path(outdir,"03_final_hypotheses.csv"),
...
# ------------------------------------------------------------
# Iteratively remove singleton FE cells
# ------------------------------------------------------------
...
drop_singletons <- function(df, country_var, year_var) {
  ...
  repeat {
    ...
    if(length(bad_c)) {
      x <- x %>%
        filter(!(.data[[country_var]] %in% bad_c))
    }
...
build_sample_and_formula <- function(d, block, y, g) {
  ...
  if(block=="financial" && y=="maturity_years") {
    controls <- c(
      controls,
      intersect("grace_period_years",names(d))
    )
  }
...
for(i in seq_len(nrow(targets))) {
  ...
  if(nrow(pruned) < 40) {
    boot_res[[hid]] <- tibble(
      ...
      reason="Too few observations after singleton pruning."
    )
    next
  }

  # Verify no singleton remains
  ...
  # Do not let fixest silently remove any more FE levels
  m <- feols(
    f,
    data=pruned,
    fixef.rm="none"
  ...
  # Fallback: cluster vector directly if clustid string fails
  if(inherits(bt,"error")) {
    ...
    if(inherits(bt2,"error")) {
      boot_res[[hid]] <- tibble(
        ...
        seed=20260828L,
        reason=paste0(
          "String clustid error: ",msg1,
          " | Vector clustid error: ",conditionMessage(bt2)
        )
      )
      next
    } else {
  ...
  boot_res[[hid]] <- tibble(
    ...
    reason=ifelse(
      is.finite(pboot),
      NA_character_,
      "boottest ran but p-value extraction failed; inspect print/summary files."
    )
  )
}
