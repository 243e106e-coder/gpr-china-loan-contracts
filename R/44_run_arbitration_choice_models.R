...
for(y in outcomes) {
  for(g in gpr_vars) {
    ...
    res[[nm]] <- broom::tidy(m,conf.int=TRUE) %>%
      mutate(
        model="arbitration_choice_lpm_twfe",
        outcome=y,
        gpr_measure=g,
        nobs=nobs(m),
...
ans <- bind_rows(res)
write_csv(ans,file.path(outdir,"09_arbitration_choice_results.csv"))

if(nrow(ans)>0) {
  write_csv(
    ans %>% filter(term %in% gpr_vars),
    file.path(outdir,"10_arbitration_gpr_coefficients.csv")
  )
}

message("44_run_arbitration_choice_models.R completed.")
